...
def train(batch_size, epochs):
    ...
    save_dir = './model_IHC_Inception_unet_args'
    ...
    X_train = np.load('Train_IHC/imgs_train.npy')
    X_train = X_train.astype('float32')
    y_train = np.load('Train_IHC/imgs_mask_train.npy')
    y_train = np.expand_dims(y_train,-1)

    y_train = y_train.astype('float32')
    y_train /= 255.  # scale masks to [0, 1]

    X_valid = np.load('Train_IHC/imgs_valid.npy')
    X_valid = X_valid.astype('float32')
    y_valid = np.load('Train_IHC/imgs_mask_valid.npy')

...
class Slidingpatches:
    ...
    def merge_patches(self, patches):
        ...
        #     label = 1
        #image = np.zeros((img_h, img_w, patches.shape[3]), dtype=np.float)  ##pp
        #sum_c = np.zeros((img_h, img_w, patches.shape[3]), dtype=np.float)  ##pp
        image = np.zeros((img_h, img_w), dtype=np.float)
        sum_c = np.zeros((img_h, img_w), dtype=np.float)
        iter_tot = 0
        for h in range(int(math.ceil((img_h - img_patch_h) / stride_h + 1))):
            for w in range(int(math.ceil((img_w - img_patch_w) / stride_w + 1))):
                start_h = h * stride_h

...
t=read.csv("validData_DCIS_DAVE.csv")
pdf(file = "Fig4v3.pdf", width = 18 , height = 8)
...
#t=read.csv("annotation_area_v2.csv")
#pdf(file = "Fig4d.pdf", width = 18 , height = 8)
p2 <- ggscatter(t,x="B.Area.IHC",y="A.Area.HE",add="reg.line",conf.int=TRUE,cor.coef = TRUE,cor.method = "pearson",xlab="Biological DCIS estimation",ylab="Automated DCIS estimation")
p3<-grid.arrange(p1,p2, nrow=1)
dev.off()

#Figure 6

MS_data=read.csv('script_Fig6_data.csv',stringsAsFactors = F)
MS_data_PD=MS_data[MS_data$Group == "Pure DCIS",]
MS_data_AD=MS_data[MS_data$Group == "Adjacent DCIS",]
pdf(file="MS_Lym_Pure_adjacent_Figure6.pdf",width = 12 , height = 8)

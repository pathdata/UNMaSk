function ret = Retinex(rgb)

rgb = log(double(rgb) + 1);

nIterations = 4;
Retinex1 = retinex_frankle_mccann(rgb(:,:,1), nIterations);
Retinex2 = retinex_frankle_mccann(rgb(:,:,2), nIterations);
Retinex3 = retinex_frankle_mccann(rgb(:,:,3), nIterations);
ret = uint8(exp(cat(3,Retinex1,Retinex2,Retinex3))-1);


clear RR IP OP NP Maximum;
end

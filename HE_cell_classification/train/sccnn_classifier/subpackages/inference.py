...
def soft_max(in_tensor, name):
    ...
    with tf.name_scope(name):
        ...
        sum_mask = tf.reduce_sum(exp_mask, 3, keepdims=True)
        soft_mask = tf.math.divide(exp_mask, sum_mask)
        return soft_mask
...
def inference(network, images=None, is_training=False):
    if images is None:
    ...
    with tf.name_scope('Dropout'):
        if is_training:
            dropout_4 = tf.nn.dropout(r_convolution_4, rate=0.5)
        else:
    ...
    logits = tf.squeeze(softmax, axis=[1, 2])
